...
class HabitCRUDTests(APITestCase):
    ...
    def setUp(self):
        ...
        # Создаем тестовые привычки с корректной длительностью (≤ 120 секунд)
    ...
    def test_get_public_habits(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        public_habits = [habit for habit in response.data['results']
                         if habit['is_public']]
    ...
    def test_create_habit_with_reward_and_linked_habit(self):
        ...
        data.update({
            'reward': 'Кофе',
            'linked_habit': self.habit1.id  # Используем существующий ID привычки
        })
    ...
    def test_habit_ordering(self):
        ...
        response = self.client.get(self.habits_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # Проверяем, что привычки отсортированы по created_at (desc)
