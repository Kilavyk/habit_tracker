...
class Habit(models.Model):
    ...
    frequency = models.PositiveIntegerField(
        default=1,
        verbose_name=_('периодичность (в днях)')
    )
    ...
    last_reminder_sent = models.DateTimeField(
        verbose_name=_('последнее напоминание отправлено'),
        null=True,
        blank=True
    )
    ...
    def clean(self):
        if self.duration > 120:
            raise ValidationError(
                _('Время выполнения не может превышать 120 секунд.')
            )

        if self.frequency > 7:
            raise ValidationError(
                _('Периодичность не может быть реже раза в неделю.')
            )
